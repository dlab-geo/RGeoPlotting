...
- Explore basic mapping techniques the sample data provided by the maps package.
- Take an introductory look at methods for working with spatial data objects and mapping them with R's base plotting system.
...
- Open RStudio or similar program to get started with this tutorial.
- Download the data that the online GitHub repository for this tutorial: 
  - Go to: https://github.com/pattyf/RGeoPlotting
  - Click on the **Download zipfile** link to download all the tutorial files
...
<br>
Set your working directory (or folder) to the directory on your computer that contains the tutorial data. This is where R will write ouput files and look for input.
...
- **long** is X cooridnate value, typically longitude in decimal degrees as a numeric data type. Longitude values range from +-180 degrees east or west of prime meridian (running through Greenwich, England - just outside of London).
- **lat** is the Y coordinate value, typically latitude. Latitude values range from +-90 degrees north or south of the equator.
...
- The data mapped with geographic coordinates creates distortion dramatically in the east-west direction resulting in a stretched out looking North America. This stretched look is clue that geographic coordinates are being used to create a map.
- The data mapped with the mercator map projection looks better for USA but grossly exaggerates areas in all directions as you get closer to the poles, in this case the north pole.
- Some software recognizes geographic coordinates and will apply a map projection transformation to improve the display of the data.
...
It's best practice to map and analyze spatial data that are in a projected not geographic coordinate reference system.
