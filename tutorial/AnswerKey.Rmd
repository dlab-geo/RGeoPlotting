...
- Students should be encouraged to bring a laptop to the workshop with R, RStudio & packages installed.
...
- Download the tutorial data that the online GitHub repository for this tutorial: 
  ...
  - Or (if you have git installed) do File -> New Project, select Version Control, then Git, then paste the github URL into the repository box.
...
Set your working directory (or folder) to the directory on your computer that contains the tutorial data. This is where R will write output files and look for input. Or if you have created an Rstudio project (File -> New Project) for this training you won't need to change the directory.

```
setwd("~/Documents/Dlab/SpatialPlot")   # or local folder on Mac / *nix computer
#setwd("~/Dropbox/Dlab/SpatialPlot")    # Could also be a dropbox folder (smart!)
#setwd("c:/docs/Dlab/SpatialPlot")      # MS Windows OS stye
```
...
In brief, geographic data are data that specify locations on or near the surface of the earth and may also include attributes that describe that location. Digital geographic data are also called geospatial or spatial data. Geographic data in R are can be stored in one of several classes of data objects. The **ggplot** package requires geographic data to be in an R data frame, while other packages require specific spatial data objects, such as SP package SpatialPointsDataFrame. A spatial data object in R (or in other software) specifies locations using spatial data types (points, lines, polygons, grids), provides support for coordinate reference system information to locate geographic data on the surface of the earth, and has spatial functions for operating on these data, for example to calculate spatial metrics like area and distance or spatial relations like intersects, contains, touches, etc. While spatial data objects facilitate more powerful spatial analyses. In this tutorial you will create maps from geographic data in both data frames and spatial objects. Moreover, we can transform geographic data from one format to the other in R.
...
# Same calculation with a single function:
sum(world$region == "Canada")
```
...
That said, here are a few important notes about map projections & coordinate refeference Systems (CRS):
...
- All projections introduce distortion in shape, area, distance and,or direction. Map projections are specified to minimize one or more of these sources of distortion for one region.
...
Read the package help doc on the rColorBrewer Package
...
Change the fill palette and the outline color for the previous map. Experiment with several of the brewer palettes.

## Section 2: Creating choropleth maps

Choropleth maps symbolize areas by values. Thus, they are an important tool for combining spatial and attribute data. To create a choropleth map you need to link the attribute data to the spatial data (polygons).  We will do this with in the following section by loading data for US states from the maps package and then linking the data frame of state attributes to the state polygons based on state id or state name.
...
```{r, eval=FALSE}
cafe <- read.csv("cafe.csv") # or cafe <- read.csv("data/cafe.csv")
head(cafe)
str(cafe)
```
...
Thus far we have mapped data where the geographic locations are stored in a data frame as numbers in columns labeled *lat* and *lon*. We will now work with explicit spatial data which store geographic locations as points, lines and polygons objects. FYI here is a really nice cheat sheet for working with spatial data in R:
...
# Don't use class as a variable name as it is reserved word.
