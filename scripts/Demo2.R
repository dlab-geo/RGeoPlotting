...
ggmap(bayarea_map) +
  geom_point(data=us.cities, aes(x=long, y=lat, size=us.cities$pop), 
  shape=21, fill="red",color="white", alpha=.75) +
  scale_size_continuous(range=c(2,60)) #try changing these

#remove legend by adding:  theme_nothing(legend=TRUE)
ggmap(bayarea_map) +
  geom_point(data=us.cities, aes(x=long, y=lat, size=us.cities$pop), 
             shape=21, fill="red",color="white", alpha=.75) +
  #scale_size_continuous(range=c(2,60)) +
  continuous_scale("area")
